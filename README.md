...
`binaryClass.r` and `Proyect2.r` are two near-identical iterations of the same end-to-end pipeline (`Proyect2.r` is the more complete, later version — it adds a row-count sanity check and returns the PCA variance-explained table). Either one runs the full pipeline standalone:
...
```
binaryClass.r      # Full pipeline: setup, EDA, PCA, modeling, evaluation (earlier iteration)
Proyect2.r          # Same pipeline, later/more complete version
...
Open either `.r` script in RStudio (or run with `Rscript`) and run it top to bottom — each is self-contained. Both call a `verificar_paquetes()` helper at the top that automatically installs any of the required packages that aren't already present, so a fresh R environment is enough to get started. Update the `setwd('PATH')` call at the top of each script to point at this project's folder before running (paths to `GUIA-1.xlsx` inside the scripts are already relative).

## Notes from a static review

R isn't available in the environment used to verify this portfolio, so these scripts could not be executed end-to-end — the review below is from reading the code, not from a live run.

- Fixed: both scripts had `GUIA-1.xlsx` and (in `Proyect2.r`) `setwd()` hardcoded to an absolute path on the original author's machine — changed to a relative path / placeholder so they work from this folder.
